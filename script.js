...
let logos = [
    {
        ...
        imageUrl: "https://cdn.pixabay.com/photo/2017/03/16/21/35/abstract-2152020_1280.png",      
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const checkFirebase = setInterval(() => {
        if (window.firebaseServices) {
            ...
            console.log("Firebase services detected, initializing app...");
            initApp();
        }
    }, 100);
});
...
function initApp() {
    // Get Firebase services from the global object
    const { db, storage, firestore, storageAPI } = window.firebaseServices;
    ...
    async function loadLogosFromFirebase() {
        try {
            ...
            const logosCollection = firestore.collection(db, 'logos');
            const q = firestore.query(logosCollection, firestore.orderBy('id', 'desc'));
            ...
            if (loadedLogos.length > 0) {
                ...
                for (const logo of logos) {
                    try {
                        await firestore.addDoc(firestore.collection(db, 'logos'), logo);
    ...
    async function handleLogoUpload(event) {
        ...
        try {
            ...
            const storageRef = storageAPI.ref(storage, `logos/${fileName}`);
            
            console.log("Uploading file to Firebase Storage...");
            // Upload file to Firebase Storage
            const snapshot = await storageAPI.uploadBytes(storageRef, file);
            ...
            const downloadURL = await storageAPI.getDownloadURL(snapshot.ref);
            ...
            await firestore.addDoc(firestore.collection(db, 'logos'), newLogo);
